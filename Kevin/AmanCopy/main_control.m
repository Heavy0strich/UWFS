%% main_control
% Purpose:
%   main program to simulate vehicle dynamics behavior under braking scenarios
%   minor steering maneuvers, and throttle scenarios. Includes algorithms
%   for control of braking. 
% Inputs:
%   Steering angle, brake torque, throttle torque
%   Control: desired velocity profile
% Outputs:
%   COG velocities, position, orientation, acceleration, wheel dynamics,
%   aerodynamic forces, 
%
...
Iveh            = 240;                  % vehicle moment of inertia (manually computed in 'mass2.xlsx' and inserted here
DRS             = 1;                    % Turn on DRS [0=off, 1=on]
ChassisDRAG     = 1;                    % Turn on aero drag effects on chassis model [0=off, 1=on]


%###################################################################
%##################  Input Profiles/ User Input  ###################
%###################################################################
%%%% Save data to export or print immediately
measuredflag=inptdf('\n\nSelect data output: 1. Print in program, 2. Export,[1]',1);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   Control   %%%%%%%%%%%%%%%%%%%%%%%%%
%%%% Brake Torque Profile
incl_WTB = 0;   %Flag to include brake torque data at end of sim.
% Specify if using control or not. If not specify if manual brake torque
% input or not:
ControlFlag=inptdf('\n\nChoose if BRAKE CONTROL input: 1 = Yes,  0 = No,[1]',1);
if ControlFlag == 1
    % Specify velocity profile:
    %%% velocity profile generator here
    % We are assuming here that the entire velocity profile is available
    % from the beginning. In reality the MPC/path planner will generate
    % velocity profile for a short time horizon. However, at the controller
    % level, it will see only one or two values. For simulation, it is
    % acceptable to have the entire profile from the start. 
    
    % W_TB will be computed within main loop since the algorithm relies on
    % other inputs as well as the velocity reference
    k           = 2;        % Lyapunov gain (vehicle controller)
    lamb        = 2.5;        % Lyapunov gain (brake hydr. controller)
    vREF        = [15;15];     % step input. must give at least two values
    tsample     = 0.015;    % sensor sample time for ADC
    DZ          = 0.1;      % Control deadzone (tolerance for applying control)
...
if MeasuredFlag2 == 1
    ...
    FW_Ang = SP4(t,TStart,TStop,Amp); % Load custom brake torque profile
    incl_STEER = 1;
...
%%%% Throttle Profile  %%%%%%%%%%%%%%%%%%% (Need to build generator function)
incl_THROT = 0;   
% MeasuredFlag3=inptdf('\n\nChoose if THROTTLE input: 1. Yes, 2. No,[1]',1);
% if MeasuredFlag3 == 1
%     TStart=inptdf('\n\nEnter THROTTLE start time in (sec.), (1)', 1);
%     TStop=inptdf('\n\nEnter THROTTLE stop time in (sec.), (2)', 2);
%     Amp=inptdf('\n\nEnter THROTTLE peak amplitude in (Nm), (50)', 50);
% 
%     W_THROT = XXXX(t,TStart,TStop,Amp,Bias); %% REPLACE WITH GENERATOR FUNCTION
% 
%     incl_THROT = 1;
% elseif MeasuredFlag3 == 2
%     W_THROT = XXXX(t,TStart,TStop,0); %% REPLACE WITH GENERATOR FUNCTION
% end
...
XYc(:,1)             = [X0;Y0];
VX0                 = Vcogx0*cos(PsiA0)-Vcogy0*sin(PsiA0);  % Recompute initial velocities to match w0 and remove minute error in LSR due to machine accuracy
...
FL_static           = car.mass * g * (car.cgdistance * 0.5);% Uses more symmetrical numbers instead of lateral distance values
...
%%%%%%%%%%%%%%%%% CONTROL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (ControlFlag == 1)
    W_TB            = zeros(4,length(t));                   % torque values [Nm]
    TBREF           = zeros(2,length(t));                   % reference torque value generated from vehicle controller [Nm]
    P_MC            = zeros(2,length(t));                   % control input values [kPa] 
    Pw              = zeros(2,length(t));                   % wheel brake pressure [kPa]
    V0              = zeros(2,length(t));                   % Brake fluid volume displace [cm^3]
    dpdv            = zeros(2,length(t));                   % slope of Brake Capacitance curve                                       

    whatisi = zeros(1,length(t ));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(t)
    if Vcog(1,i)>0
        %Compute Aerodynamic forces and transform to vertical loads
        if(DRS==1)
            dragCd = car.DRS_cD;
            liftCl = car.DRS_cL;
        else
        ...
        aeroLift        = aeroForce(liftCl, car.frontArea, Vcog(1,i));
        aeroDrag        = aeroForce(dragCd, car.frontArea, Vcog(1,i));
        ...
        % Compute tire forces, slip ratios/angles, etc.
        if i+1 <= length(t)
...
%%%%%%%%%%%%%%%%%%%%%%%% CONTROL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if ControlFlag == 1
            % "ADC"
            % Convert "analog" feedback signals to digital representation.
            % The ADC function is not implemented here. Instead, we hold
            % the signal value constant after each sampling time interval.
            % This is to mimic the function of an ADC. However, it is in
            % the best interest in the future to simulate an ADC function
            % as it can also introduce errors through quantization.
            tmod = mod(t(i),tsample);   % computes remainder of sim time/ sample time
            if(tmod == 0)               % sampling happens when sim time is divisible by sample time
                itemp = i+1;              % temp variable to store index at sampling 
                
                VcogDIG = Vcog(1,i);    % The 'discrete' sampled value. Only need x-component
                
                if i+1 <= length(t)
                    % Compute control input (Pressure at Master Cylinders)
                    % Computed only when sampling
                    [P_MC(:,i+1),dpdv(:,i),TBREF(:,i+1)] = brake_control(k,lamb,vREF,car,Brakes,VcogDIG,DRS,Pw(:,i),P_MC(:,i),...
                        0,0.15,1,R,tsample,DZ);
                end
            elseif(tmod ~= 0)          
                % If there is a remainder then it indicates that sampling
                % is not taking place. Store previously computed P_MC to
                % generate square waveform and to simplify indexing within
                % simulation. In a true digital system only the sampled
                % value and result would be held
                if i+1 <= length(t)
                    P_MC(:,i+1) = P_MC(:,itemp);
                    TBREF(:,i+1) = TBREF(:,itemp);
                end
                whatisi(i) = itemp;
            end
            ...
            % Use Zero-Order-Hold (ZOH). Generates 'analog' square wave
            % form (e.g. converts single discrete point to a square wave
            % with same aplitude but now covers a sample period)
            % Since P_MC is already stored above and held constant over a
            % sampling period, the DAC role is already done and nothing is
            % added here.

            % Compute "actual" brake torque through brake hydraulic model
            [W_TB(:,i), Pwout,Vout] = brake_hydraulic_model(Pw(:,i),P_MC(:,i),V0(:,i),Vcog(1,i),car,Brakes,delt);
            if i+1 <= length(t)
                Pw(:,i+1) = Pwout;
                V0(:,i+1) = Vout;
            end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if i+1 <= length(t)
            % Include throttle profile in wheel_speed() once created
            [w_w(:,i+1),w_acc(:,i+1)]=wheel_speed(Fl(:,i),0,W_TB(:,i),Bd,0.15,R,delt,w_w(:,i));
        end        
        
        % Transform Long./Lat forces to undercarriage force components
        ...
        % Nonlinear 2D Chassis model
        if i+1 <= length(t)
            x0 = [Vcog(1,i);Vcog(2,i);YawP(1,i);YawR(1,i);XYc(1,i);XYc(2,i)];
            [vcog,acog,beta,yaw,xym,vxym,psi] = PlantModel_2D(FX(:,i).',FY(:,i).',x0,car.mass,...
                Iveh,car.cg2faxle,car.cg2raxle,car.wheelbase,delt,DragFX);
    ...
    elseif Vcog(1,i)<=0  %% end simulation if vehicle achieves stopped position or tries to reverse
        break
    end
end
...
y2 = ones(1,length(t)) * vREF(1,1);
plot(t,y2,'--r')
legend('Vcog','Vx,o','Vx,ref')
...
if(incl_THROT == 1)
figure('Name','Input: Throttle Torque Profile','NumberTitle','off')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% CONTROL %%%%%%%%%%%%%%%%%%%%%%%%%%
figure('Name','Controller: Torque Reference TREF','NumberTitle','off')
plot(t,TBREF(:,:))
xlabel('t (sec)')
ylabel('Torque (Nm)')
title('Torque Reference from Vehicle Control')

figure('Name','Control Input: P_MC')
plot(t,P_MC(1,:));
xlabel('t (sec.)')
ylabel('Pressure MC (kPa)')
title('Control Input: Master Cylinder Pressure')

figure('Name','Output: Brake Fluid Volume Displaced')
plot(t,V0(1,:));
xlabel('t (sec.)')
ylabel('Volume (cm^3)')
title('Output: Displaced Brake Fluid (Theoretical)')

figure('Name','Output: Wheel Brake Pressure','NumberTitle','off')
plot(t,Pw(1:2,:))
xlabel('t (sec)')
ylabel('Wheel Brake Pressure (kPa)')
title('Wheel Brake Pressure [front,rear]')

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

elseif(measuredflag == 2)
    %filename=inptdf('\n\nType output filename: ',VehicleSimData.mat');
    %save(filename,'t','Vcog','Acog','lSR','Fl','FX','Fc','FY','FzCOP','Beta','YawP','YawR','XYc');
    SimulationResults5Obj = matfile('SimulationResults5.mat');
    save('SimulationResults5Obj');
