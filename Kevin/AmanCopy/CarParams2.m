...
% Brake Params used in CONTROL
Kb                      = 0.5;          % Effective brake gain used in torque(wheel pressure) function.
CqFront                 = 1.49;         % Flow coefficient of hydraulic lines [cm^3/s(kPa^1/2)]
CqRear                  = 1.49;
Ppo                     = 115;          % Push out pressure [kPa] (pressure where braking begins)
Pa                      = 100;           % Pressure setting lower limit for well defined slope dPdV (Brake Capacitance function)
aFront                  = 137;          % slope at lower limit [kPa/cm^3]
aRear                   = 137; 
VolimFront              = 0.22;         % Brake capacitance volume lower limit such that the function is strictly increasing for V>Volim
VolimRear               = 0.22;

Brakes.Kb               = Kb;            
Brakes.CqFront          = CqFront;
Brakes.CqRear           = CqRear;
Brakes.Ppo              = Ppo;
Brakes.Pa               = Pa;
Brakes.aFront           = aFront;
Brakes.aRear            = aRear;
Brakes.VolimFront       = VolimFront;
Brakes.VolimRear        = VolimRear;
...
%    car.motorType           = motorType;
    ...
    car.aFront              = aFront;
    car.aRear               = aRear;
    car.VolimFront          = VolimFront;
    car.VolimRear           = VolimRear;
    

%% Max Brake Gs and Corresponding Wheel Braking Forces
[BrkG] = brakeG(runPacejka, tire, car, GTol, maxItr);

FrontForce = BrkG.BForce(3,:,1);        % Normal forces on front left wheel
RearForce = BrkG.BForce(3,:,3);         % normal forces on rear left wheel
% Torque to lock wheel:
% T = mu(tire/road) * Fz(wheel) * Radius(wheel)
LockTorque_F = FrontForce.*((car.wheel_dia/2)*car.roadCoefficient);     
LockTorque_R = RearForce.*((car.wheel_dia/2)*car.roadCoefficient);
Brakes.LkTorque_F = LockTorque_F;
Brakes.LkTorque_R = LockTorque_R;
